...
Личный сайт-портфолио frontend-разработчика с fullstack-опытом и Team Lead. Vue 3 + Vite + TypeScript, анимации на GSAP, дизайн-токены, две локали и **пререндер в статику**: каждый маршрут собирается в готовый HTML, браузер потом гидрирует ту же разметку.
